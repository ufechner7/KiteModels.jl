...
function calc_initial_loop_state(s::KPS4_3L)
    HEIGHT = MVector{length(s.springs)}(zeros(length(s.springs)))
    RHO = MVector{length(s.springs)}(zeros(length(s.springs)))
    V_WIND_TETHER = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    L_0 = MVector{length(s.springs)}(zeros(length(s.springs)))
    K = MVector{length(s.springs)}(zeros(length(s.springs)))
    C = MVector{length(s.springs)}(zeros(length(s.springs)))
    SEGMENT = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    REL_VEL = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    AV_VEL = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    NORM1 = MVector{length(s.springs)}(zeros(length(s.springs)))
    UNIT_VECTOR = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    K1 = MVector{length(s.springs)}(zeros(length(s.springs)))
    K2 = MVector{length(s.springs)}(zeros(length(s.springs)))
    C1 = MVector{length(s.springs)}(zeros(length(s.springs)))
    SPRING_VEL = MVector{length(s.springs)}(zeros(length(s.springs)))
    SPRING_FORCE = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    V_APPARENT = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    AREA = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    V_APP_PERP = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))
    HALF_DRAG_FORCE = MMatrix{3, length(s.springs)}(zeros(3, length(s.springs)))

    c_spring = s.set.c_spring ./ s.l_tethers ./ s.set.segments
    damping = s.set.damping ./ s.l_tethers ./ s.set.segments
    for i in eachindex(s.springs)
        p1 = s.springs[i].p1  # First point nr.
        p2 = s.springs[i].p2
        HEIGHT[i] = 0.5 * (s.pos[p1][3] + s.pos[p2][3])
        RHO[i] = calc_rho(s.am, HEIGHT[i])
        V_WIND_TETHER[:,i] .= calc_wind_factor(s.am, HEIGHT[i]) .* s.v_wind_gnd

        i <= s.set.segments*3 ? L_0[i] = s.segment_lengths[i%3+1] : L_0[i] = s.springs[i].length # Unstressed length
        i <= s.set.segments*3 ? K[i] = c_spring[i%3+1] * s.stiffness_factor : K[i] = s.springs[i].c_spring * s.stiffness_factor # Spring constant
        i <= s.set.segments*3 ? C[i] = damping[i%3+1] : C[i] = s.springs[i].damping                    # Damping coefficient    
        SEGMENT[:,i] .= s.pos[p1] - s.pos[p2]
        REL_VEL[:,i] .= s.vel[p1] - s.vel[p2]
        AV_VEL[:,i] .= 0.5 * (s.vel[p1] + s.vel[p2])
        NORM1[i] = norm(SEGMENT[:,i])
        UNIT_VECTOR[:,i] .= SEGMENT[:,i] ./ NORM1[i]
        K1[i] = 0.25 * K[i] # compression stiffness kite segments
        K2[i] = 0.1 * K[i]  # compression stiffness tether segments
        C1[i] = 6.0 * C[i]  # damping kite segments
        SPRING_VEL[i] = REL_VEL[:,i] ⋅ UNIT_VECTOR[:,i]

        if i > s.num_E  # kite springs
            for j in 1:3
                SPRING_FORCE[j,i] = ifelse(
                    (NORM1[i] - L_0[i]) > 0.0,
                    (K[i]*(L_0[i] - NORM1[i]) - C1[i] * SPRING_VEL[i]) * UNIT_VECTOR[j,i],
                    (K1[i]*(L_0[i] - NORM1[i]) - C[i] * SPRING_VEL[i]) * UNIT_VECTOR[j,i]
                )
            end
        else
            for j in 1:3
                SPRING_FORCE[j,i] = ifelse(
                    (NORM1[i] - L_0[i]) > 0.0,
                    (K[i]*(L_0[i] - NORM1[i]) - C[i] * SPRING_VEL[i]) * UNIT_VECTOR[j,i],
                    (K2[i]*(L_0[i] - NORM1[i]) - C[i] * SPRING_VEL[i]) * UNIT_VECTOR[j,i]
                    )
            end
        end
        V_APPARENT[:,i] .= s.v_wind_tether .- AV_VEL[:,i]
        AREA[:,i] = NORM1[i] * s.set.d_tether/1000.0
        V_APP_PERP[:,i] = s.v_apparent .- s.v_apparent ⋅ UNIT_VECTOR[:,i] * UNIT_VECTOR[:,i]
        HALF_DRAG_FORCE[:,i] .= (0.25 * RHO[i] * s.set.cd_tether * norm(V_APP_PERP[:,i]) * AREA[:,i]) .* V_APP_PERP[:,i]
    end
    return HEIGHT, RHO, V_WIND_TETHER, L_0, K, C, SEGMENT, REL_VEL, AV_VEL, NORM1, UNIT_VECTOR, K1, K2, C1, SPRING_VEL, SPRING_FORCE, V_APPARENT, AREA, V_APP_PERP, HALF_DRAG_FORCE
end