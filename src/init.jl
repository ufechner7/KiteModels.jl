...
function init_pos_vel_acc(s::KPS4, X=zeros(2 * (s.set.segments+KITE_PARTICLES)); old=false, delta = 0.0)
    ...
    # set p9
    pos[s.set.segments+1+2] .= particles[2+2] + X[j] * normalize(particles[2+2])
    vel[s.set.segments+1+2] .= [delta, delta, delta]
    acc[s.set.segments+1+2] .= [delta, delta, 0]
    ...
    pos[s.set.segments+1+3][2] += X[j+1]                     # Y position of point C
