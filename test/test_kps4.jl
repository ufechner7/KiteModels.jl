...
@testset "calc_particle_forces  " begin
    ...
    for i in 1:length(kps4.springs)
        ...
        forces = zeros((se().segments + KiteModels.KITE_POINTS + 1, 3))
        stiffnes_factor = 0.5
        v_wind_tether = [8.0, 0.1, 0.0]
        KiteModels.calc_particle_forces(kps4, pos1, pos2, vel1, vel2, v_wind_tether, spring, forces, stiffnes_factor, se().segments, se().d_tether, i)
        # println(forces)
    end
end