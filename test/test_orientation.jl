...
@testset "calc_orientation, yaw = 20°                                  " begin
    # x, y and z are given in ENU
    x = [0.9396926207859084, 0.3420201433256687, 0.0]
    y = [-0.3420201433256687, 0.9396926207859084, 0.0]
    z = [0.0, 0.0, 1.0]
    @assert is_right_handed_orthonormal(x, y, z)
    rot = calc_orient_rot2(x, y, z)
    q = QuatRotation(rot)
...
@testset "calc_orientation, pitch = 30°                                " begin
    # x, y and z are given in ENU
    x = [0.8660254037844387, 0.0, -0.49999999999999994]
    y = [0.0, 1.0, 0.0]
    z = [0.49999999999999994, 0.0, 0.8660254037844387]
    @assert is_right_handed_orthonormal(x, y, z)
    rot = calc_orient_rot2(x, y, z)
    q = QuatRotation(rot)
    roll, pitch, yaw = rad2deg.(quat2euler2(q))
...
@testset "calc_orientation, roll = 40°                                 " begin
    # x, y and z are given in ENU
    x = [1.0, 0.0, 0.0]
    y = [0.0, 0.766044443118978, 0.6427876096865393]
    z = [0.0, -0.6427876096865393, 0.766044443118978]
