...
init_sim!(s; prn=true, init_set_values=[-0.1, -0.1, -70.0])
set_values = s.integrator[s.simple_sys.set_values]
println("Set vals: ", set_values)
println("v_wind ", s.v_wind)

sys, inputs = nonlin_model!(s, s.pos, s.vel)
